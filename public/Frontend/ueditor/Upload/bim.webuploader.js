...
; (function ($, w) {
	...
	$.extend(Upload.prototype, {
		...
		postData: function () {
			...
			if (parm != '') { this.options.server += parm };
		},
		...
		initUploader: function () {
			...
			if (!self.options.uploadEvents.fileQueued) {
				self.BDUploader.on('fileQueued', function (file) {
					...
					if (self.allowNum == 0 || self.isAllowUpload() == 0) {
						...
						swal('上传文件数量超出限制', '最多上传' + self.options.fileNumLimit + '个文件', 'error');
						return false;
					}
					...
					if ($("#" + self.options.listName).length > 0) {
						...
						var liHtml = '<li class="affix-list-item" id=' + file.id + '>' + '<div class="upload-file-info">' + closeHtml + fileName + fileSize + clearfix + '</div>' + divProgress + '</li>';
			...
			if (!self.options.uploadEvents.uploadSuccess) {
				self.BDUploader.on('uploadSuccess', function (file, response) {
					...
					$fileItem.find('.upload-file-info span').eq(0).attr('data-fileid', response.msg.fileid).attr('data-fileurl', response.msg.url);
					var responseVal = '';
		...
		, delFile: function () {
			...
			$(selecter).on('click', '.webuploader-pick-file-close', function () {
				var thisClose = $(this);
				var fid = parseInt(thisClose.attr('data-fileid'), 10);
				...
				if (fid > 0) {
					$.post(self.options.delUrl, { "id": fid }, function (json) {
						if (json.status == '1') {
	...
	$.fn[pluginName].defaults = {
		...
		swf: 'Uploader.swf',
		server: '../../Handler/ProjectAddHandler.ashx',// 文件接收服务端。
		/**
		...
		delUrl: '../../Handler/ProjectAddHandler.ashx?action=del',
		separator: ',', //默认逗号
	}
