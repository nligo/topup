        ...
        return {
            init: function (selectCb) {
                uploader = new Uploader({
                    ...
                    // 文件接收服务端。
                    server: '../../Handler/UserHandler.ashx?OP=WebUserLoadavtar',
                    fileNumLimit: 1,
                    onError: function () {
            ...
            upload: function () {
                ...
                uploader.on('uploadAccept', function (file, ret) {
                    if (ret.ErrCode === 0) {
                        $.showInfoMessage(ret.ErrMessage || '头像上传成功！', 1, function () {
                            window.location.href = "Account.aspx";
                        });
                    } else {
                        $.showInfoMessage(ret.ErrMessage || '头像上传失败！', 2);
                    }
                });
            }
        }
    ...
    var Croper = (function () {
        ...
        function srcWrap(src, cb) {
            ...
            if (isBase64Supported) {
                ...
                $.ajax('../../Handler/UserHandler.ashx?OP=WebUserLoadavtar', {
                    method: 'POST',
                    data: src,
                    dataType: 'json'
